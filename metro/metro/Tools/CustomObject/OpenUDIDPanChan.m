...
@interface OpenUDIDPanChan (Private)
...
@implementation OpenUDIDPanChan
...
//    }
    ...
    if (_openUDID==nil) {
        ...
        _openUDID = [NSString stringWithFormat:
                @"%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%08llxlx",
...
//
+ (NSString*) value {
    return [OpenUDIDPanChan valueWithError:nil];
}
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [OpenUDIDPanChan _getDictFromPasteboard:slotPB];
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [OpenUDIDPanChan _generateFreshOpenUDID];
        } else {
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [OpenUDIDPanChan _setDict:localDict forPasteboard:slotPB];
    }
