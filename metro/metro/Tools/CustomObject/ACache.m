...
{
    NSData *data = [self getAsNSData:key validate:YES];
    return [ACache decodeObject:data];
}
...
- (void)put:(NSString*)key object:(NSDictionary*)object;
{
    [self put:key object:object timeout:0];
}
- (void)put:(NSString*)key object:(NSDictionary *)object timeout:(NSTimeInterval)time;
{
    NSData *data = [ACache encodeObject:object];
    
    [self put:key data:data timeout:time];
}

- (void)put:(NSString*)key string:(NSString*)data;
{
    [self put:key data:[data dataUsingEncoding:NSUTF8StringEncoding] timeout:0];
}
- (void)put:(NSString*)key data:(NSData*)data;
{
    [self put:key data:data timeout:0];
}
- (void)put:(NSString*)key string:(NSString *)data timeout:(NSTimeInterval)time;
{
    NSString *tmp = [NSString stringWithFormat:@"%@",data];
    [self put:key data:[tmp dataUsingEncoding:NSUTF8StringEncoding] timeout:time];
}
