...
#define AES_KEY 16      //AES密钥的长度（16，24，32）
...
- (NSMutableDictionary*)LcEncrypt:(NSDictionary*)EncryptDic
{
    ...
    //step5 随机生成数字和字母组成的AES密钥
    NSString *merchantAesKey = [LcAESRSA randomString:AES_KEY];
    NSLog(@"\rAES key :%@",merchantAesKey);
    //step6 用AES密钥加密含有签名的报文
    NSError *error = nil;
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:dicWithSign options:NSJSONWritingPrettyPrinted error:&error];
    NSString *realdata = [[NSString alloc]initWithData:jsonData encoding:NSUTF8StringEncoding];
...
- (NSMutableDictionary*)LcDecode:(NSDictionary*)EncryptDic
{
    ...
    NSData *jsonData = [StringybRealData dataUsingEncoding:NSUTF8StringEncoding];
    NSError *error = nil;
    NSMutableDictionary *dicData = [NSJSONSerialization JSONObjectWithData:jsonData options:NSJSONReadingMutableContainers error:&error];
