...
- (void)startWithTime:(NSInteger)timeLine title:(NSString *)title countDownTitle:(NSString *)subTitle mainColor:(UIColor *)mColor countColor:(UIColor *)color {
    ...
    color = [UIColor colorWithRed:0.32 green:0.32 blue:0.32 alpha:1];
    ...
    dispatch_source_set_event_handler(_timer, ^{
        ...
        if (timeOut <= 0) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                weakSelf.backgroundColor = mColor;
                [weakSelf setTitle:[NSString stringWithFormat:@"%@(%@)", subTitle, timeStr] forState:UIControlStateNormal];
                [weakSelf setTitleColor:color forState:UIControlStateNormal];
